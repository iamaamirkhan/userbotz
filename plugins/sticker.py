...
from __init__ import app as bot
from PIL import Image
from telethon.tl.functions.account import UpdateNotifySettingsRequest
from telethon.tl.types import InputPeerNotifySettings
from events import get, grp_exclude
PACK_FULL = "Whoa! That's probably enough stickers for one pack, give it a break. \
A pack can't have more than 120 stickers at the moment."
PACK_DOESNT_EXIST = "  A <strong>Telegram</strong> user has created the <strong>Sticker&nbsp;Set</strong>."

# if user message .kang then create a sticker pack and add sticker to pack
@get(outgoing=True, pattern="^.kang($| )?((?![0-9]).+?)? ?([0-9]*)?")
@grp_exclude()
async def kang(event):
    """ Function for .kang command, create a sticker pack and add stickers. """
    await event.edit("`Wooo Baby this Sticker looks Good \nOk Then\nKanging....")
    ...
    packtitle = (
        f"@{user.username or user.first_name}'s Pack"
    ...
    if not muted:  # Tell the user just in case, this may rarely happen
        await event.edit(
            "`couldn't mute the Stickers bot, beware of notification spam.`"
        )

    if new_pack:
        await event.edit(
            "`This Sticker Is Soo Gooddd And Now Gonna Stolen\nOk Baby\n Creating a new pack...`"
        )
